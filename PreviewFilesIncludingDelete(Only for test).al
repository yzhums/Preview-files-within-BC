            ...
            action(RunHyperLinkInBC)
            {
                ...
                begin
                    ...
                    if DocAttach.FindFirst() then begin
                        ...
                        PreviewFiles.SetURL(URL, SharePointFileID);
                        PreviewFiles.Run();
                    end;
                end;
            }
...
page 50111 "Preview Files"
{
    ...
    layout
    {
        area(content)
        {
            ...
            group(FileInfo)
            {
                ...
                field(SharePointFileID; SharePointFileID)
                {
                    ApplicationArea = All;
                    Caption = 'SharePoint File ID';
                    Editable = false;
                    ToolTip = 'SharePoint File ID';
                }
            }
        }
    }
    ...
    var
        PreviewDocument: Codeunit PreviewDocument;
    begin
        PreviewDocument.DeleteFileInSharePoint(SharePointFileID);
    end;

    procedure SetURL(NavigateToURL: Text; FileID: Text)
    begin
        URL := NavigateToURL;
...
codeunit 50102 PreviewDocument
{
    ...
    procedure DeleteFileInSharePoint(SharePointFileID: Text)
    var
        HttpClient: HttpClient;
        HttpRequestMessage: HttpRequestMessage;
        HttpResponseMessage: HttpResponseMessage;
    ...
    begin
        ...
        // application permissions (replace with the actual site-id, drive-id, folder path and file name)
        SharePointFileUrl := 'https://graph.microsoft.com/v1.0/sites/5b3b7cec-cbfe-4893-a638-c18a34c6a394/drive/items/' + SharePointFileID;
        // Initialize the HTTP request
        HttpRequestMessage.SetRequestUri(SharePointFileUrl);
